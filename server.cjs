...
// ─── Meta paginated fetch helper ─────────────────────────────────────────────
async function fetchMetaAllPages(url) {
  let rows = [];
  let nextUrl = url;
  while (nextUrl) {
...
app.get('/api/meta-adsets', async (req, res) => {
  ...
  const token     = process.env.META_ACCESS_TOKEN;
  try {
    const fields = 'campaign_name,adset_name,date_start,impressions,reach,spend,actions';
    const url = `https://graph.facebook.com/v19.0/${accountId}/insights?fields=${fields}&time_range={"since":"${startDate}","until":"${endDate}"}&time_increment=1&level=adset&limit=500&access_token=${token}`;
    const data = await fetchMetaAllPages(url);
