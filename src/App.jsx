...
function LiveSection({ campaigns, title, accent, metricKeys, charts, tableFmt, overrideTotals }) {
  const { startDate, endDate, compareMode } = useDash();
  const bp = useBreakpoint();
  ...
  const { tc, tp } = useMemo(() => {
    ...
    return { tc: { ...tc, ...(overrideTotals||{}) }, tp };
  }, [campaigns, startDate, endDate, compareMode, metricKeys, overrideTotals]);
...
function useGA4Data() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/api/ga4?start=${startDate}&end=${endDate}`)
      .then(r => r.json())
      .then(json => {
        // Handle both old (array) and new ({ rows, userTotals }) format
        const rows      = Array.isArray(json) ? json : (json.rows || []);
        const userTotals = Array.isArray(json) ? null : (json.userTotals || null);

        // Build deduplicated total users by summing date-level totals
        const totalUsers = userTotals
          ? userTotals.reduce((sum, d) => sum + d.users, 0)
          : null;
        ...
        <>
          ...
          {!loading && (
            <LiveSection campaigns={liveGA4Campaigns} {...SECTIONS.analytics}
              overrideTotals={ga4Data?.totalUsers != null ? { users: ga4Data.totalUsers } : undefined}/>
          )}
          <CountrySection/>
          <div style={{ height:1, background:B.border, margin:"32px 0 28px" }}/>
          <SearchTermsSection/>
...
function MetaPaidTab() {
  ...
  const campaignAgg = aggregateRows(adsets||[], 'campaign');
  ...
  return (
    <>
      ...
      <div style={{ marginTop:28 }}>
        ...
        {(loadingDrill || adsets === null) ? <Spinner accent={accent} label="DRILL-DOWN"/> : (
          <div style={{ border:`1px solid ${B.border}`, borderRadius:6, overflow:"hidden" }}>
            {campaignAgg.map((camp, ci) => {
              const campOpen = selectedCampaign === camp.name;
              const adsetAgg = aggregateRows((adsets||[]).filter(r => r.campaign === camp.name), 'adset');

              return (
                <div key={camp.name} style={{ borderBottom: ci < campaignAgg.length-1 ? `1px solid ${B.border}` : "none" }}>
                  {/* Campaign row */}
                  <div onClick={() => setSelectedCampaign(campOpen ? null : camp.name)}
                    style={{ display:"flex", alignItems:"center", padding:"11px 14px", cursor:"pointer",
                      background: campOpen ? B.surface : "transparent", transition:"background 0.15s" }}
                    onMouseEnter={e=>{ if(!campOpen) e.currentTarget.style.background=B.surface; }}
                    onMouseLeave={e=>{ if(!campOpen) e.currentTarget.style.background="transparent"; }}>
                    <span style={{ fontSize:12, color:accent, marginRight:10, transition:"transform 0.2s",
                      display:"inline-block", transform: campOpen?"rotate(90deg)":"rotate(0deg)" }}>›</span>
                    <span style={{ flex:1, fontFamily:"'Barlow Condensed',sans-serif", fontWeight:700,
                      fontSize:12, color:B.textPrimary }}>
                      {camp.name}
                    </span>
                    {DRILL_METRICS.filter(k=>["impressions","spend","leads"].includes(k)).map(k => (
                      <span key={k} style={{ marginLeft:20, fontFamily:"'Barlow Condensed',sans-serif",
                        fontSize:11, color:B.textMuted }}>
                        <span style={{ color:B.textSecond, fontWeight:600 }}>{fmtDrillVal(k, camp[k]||0)}</span>
                        <span style={{ fontSize:9, marginLeft:4, textTransform:"uppercase", letterSpacing:"0.08em" }}>{DRILL_LABEL[k]}</span>
                      </span>
                    ))}
                  </div>
                  ...
                  {/* Ad Set accordion */}
                  {campOpen && (
                    <div style={{ background:B.bg, borderTop:`1px solid ${B.border}` }}>
                      {adsetAgg.map((adset, ai) => {
                        const adsetOpen = selectedAdset === adset.name;
                        const adAgg = aggregateRows((ads||[]).filter(r => r.campaign === camp.name && r.adset === adset.name), 'ad');

                        return (
                          <div key={adset.name} style={{ borderBottom: ai < adsetAgg.length-1 ? `1px solid ${B.border}` : "none" }}>
                            {/* Ad set row */}
                            <div onClick={() => setSelectedAdset(adsetOpen ? null : adset.name)}
                              style={{ display:"flex", alignItems:"center", padding:"10px 14px 10px 32px",
                                cursor:"pointer", background: adsetOpen ? B.surface : "transparent", transition:"background 0.15s" }}
                              onMouseEnter={e=>{ if(!adsetOpen) e.currentTarget.style.background=B.surface; }}
                              onMouseLeave={e=>{ if(!adsetOpen) e.currentTarget.style.background="transparent"; }}>
                              <span style={{ fontSize:11, color:B.textMuted, marginRight:10, transition:"transform 0.2s",
                                display:"inline-block", transform: adsetOpen?"rotate(90deg)":"rotate(0deg)" }}>›</span>
                              <span style={{ flex:1, fontFamily:"'Barlow Condensed',sans-serif", fontWeight:600,
                                fontSize:11, color:B.textSecond }}>
                                {adset.name}
                              </span>
                              {DRILL_METRICS.filter(k=>["impressions","spend","leads"].includes(k)).map(k => (
                                <span key={k} style={{ marginLeft:20, fontFamily:"'Barlow Condensed',sans-serif",
                                  fontSize:10, color:B.textMuted }}>
                                  <span style={{ color:B.textSecond, fontWeight:600 }}>{fmtDrillVal(k, adset[k]||0)}</span>
                                  <span style={{ fontSize:9, marginLeft:4, textTransform:"uppercase", letterSpacing:"0.08em" }}>{DRILL_LABEL[k]}</span>
                                </span>
                              ))}
                            </div>
                            ...
                            {/* Ad level */}
                            {adsetOpen && (
                              <div style={{ background:B.surface, borderTop:`1px solid ${B.border}`, padding:"12px 14px 12px 48px" }}>
                                <DrillCharts rows={(ads||[]).filter(r => r.campaign === camp.name && r.adset === adset.name)}
                                  groupKey="ad" startDate={startDate} endDate={endDate} accent={accent}/>
                                <DrillTable rows={adAgg} accent={accent} onRowClick={null}/>
                              </div>
                            )}
                          </div>
                        );
